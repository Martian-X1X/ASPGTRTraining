...
namespace ASPGTRTraining.MVC.Controllers
{
    public class HomeController : Controller
    {
        ...
        public HomeController(ILogger<HomeController> logger, ASPGTRTrainingDBContext db)
        {
            _logger = logger;
            this.db = db;
        }
        ...
        public async Task<IActionResult> DepartmentSave(string Id)
        {
            ...
            var edit= await db.Departments.Where(x => x.Id == Id).SingleOrDefaultAsync();
            if (edit is null)
            {
                return NotFound();
            }
        ...
        public async Task<IActionResult> DepartmentSave(DeptListDTO model)
        {
            ...
            {
                var edit = await db.Departments.Where(x => x.Id == model.Id).SingleOrDefaultAsync();
                if (edit is null)
                {
                    return NotFound();
                }
